...
class DecisionsScreenState extends State<DecisionsScreen> {
      ...
      setState(() {
        if (value != null) {
          ...
          if(decisonResponseModel!=null&&decisonResponseModel.voters!=null){
            ...
            // allComents=decisonResponseModel.comments;
  ...
  // String getStatus(){
  // if(approved.length>abstained.length&& approved.length>denied.length&&approved.length>pending.length)  {
  //   status="Approved";
  //   colorStatus=Colors.green;
  // }else if(abstained.length>approved.length&& abstained.length>denied.length&&abstained.length>pending.length)  {
  //   status="Abstained";
  //   colorStatus=Color(0xff0C64F9);
  // }else if(denied.length>approved.length&& denied.length>abstained.length&&denied.length>pending.length)  {
  //   status="Denied";
  //   colorStatus=Color(0xffFF6A81);
  // }else if(pending.length>approved.length&& pending.length>abstained.length&&pending.length>denied.length)  {
  //   status="Pending";
  //   colorStatus=Color(0xffFEC20E);
  // }
  // return status;
  ...
  void addComment(String token,String note) {
    load();
    meetingRepository = new MeetingRepository();
    Future<List<AddCommentResponseModel>> allList = meetingRepository.addComment(token,widget.decisionId,new AddCommentRequestModel(comment: note));
    allList.then((value) {
      setState(() {
        if (value != null) {
          userAndCommentsList=[];
  ...
  void changeVote(String token,String vote,String reason) {
    load();
    meetingRepository = new MeetingRepository();
    Future<ChangeVoteResponseModel> allList = meetingRepository.changeVote(token,widget.decisionId,new ChangeVoteRequestModel(reason: reason,vote: vote));
    allList.then((value) {
      setState(() {
        if (value != null) {
          status=votesValue;
